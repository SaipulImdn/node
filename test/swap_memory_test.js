...
exports['Should correctly fetch swap memory information on linux'] = function(test) {
  var psUtil = new PSUtil();
  // Save the existing setup
  var platform = process.platform;
  var readFileFunction = PSUtil._readFile;
  var lib_function = psUtil._lib.swap_memory;

  // Setup Overrides
  process.platform = 'linux'
  PSUtil._readFile = function(path, encoding, callback) {
    fs.readFile('./test/linux/vmstat.txt', encoding, callback);
  }
  ...
  psUtil.swap_memory(function(err, result) {
    ...
    psUtil._lib.swap_memory = lib_function;
    test.done();
  });
}